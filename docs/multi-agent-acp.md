...
Lingo 的“代码”模式复用同一套 ACP client、timeline、权限、附件、工作区和会话恢复
链路接入外部代码 Agent，并额外提供 Lingo 原生代码会话。每个会话在创建时绑定一个
...
| Agent | 启动方式 | 运行时来源 | 登录 |
|---|---|---|---|
| Lingo （原生） | 进程内 CodeWhale Engine（非 ACP 子进程） | 与“工作”模式相同的应用模型配置 | 无独立登录，沿用工作模式的模型凭据 |
| Codex | `codex-acp` Bridge | Lingo 内置 Bridge；Codex CLI 优先使用系统安装（≥ 0.144.6），缺失或过旧时经用户确认自动安装或升级（见下文安装与升级矩阵） | Lingo 内完成 Codex OAuth；也支持 `OPENAI_API_KEY` |
| Claude Code | `claude-agent-acp` Bridge | Lingo 内置 Bridge（仅 JS 适配器，版本固定为 `0.62.0`）；Claude Code CLI 使用系统安装（≥ 2.0.0），App 不内置 CLI，缺失时经用户确认运行官方安装脚本，过旧时按安装来源升级（见下文安装与升级矩阵） | 在 Lingo 点击“授权登录”；也支持 `ANTHROPIC_API_KEY`、`ANTHROPIC_AUTH_TOKEN`、`CLAUDE_CODE_OAUTH_TOKEN` |
| Kimi | `kimi acp` | 自动检测系统 `PATH` 中的官方 Kimi Code CLI（≥ 0.9.0），缺失时经用户确认运行官方安装脚本，过旧时按安装来源升级 | 在 Lingo 点击“授权登录”，按提示完成设备码授权；也支持成对设置 `KIMI_MODEL_NAME` 与 `KIMI_MODEL_API_KEY` |

Lingo 原生会话不是 ACP 后端：它复用“工作”模式的 Engine，通过 `chat` 命令发消息、
...
语义，成品卡工具同时对代码会话隐藏）。 Lingo 原生会话可开启 Lingo 多智能体模式；
...
| Agent | 方式 | 说明 |
|---|---|---|
| Codex | 官方脚本 | macOS/Linux：`curl -fsSL https://chatgpt.com/codex/install.sh \| sh`，默认写入 `~/.local/bin`；Windows：`irm https://chatgpt.com/codex/install.ps1 \| iex`，默认写入 `%LOCALAPPDATA%\Programs\OpenAI\Codex\bin`； Lingo 使用平台绝对路径重新探测 |
...
- Lingo 负责进程托管、ACP transport、session/workspace 绑定、事件持久化、权限与 UI。
...
- 三种 ACP Agent 共用 `acp-timeline.jsonl` 和工作区安全边界，不复制三套 reducer 或文件
  操作实现； Lingo 原生会话不经过 ACP transport，历史与回合状态来自 SavedSession 与
  `timing_events.jsonl`，展示层与 ACP 会话复用同一个 ConversationTimeline。
- Kimi ACP 会把普通 provider failure 映射为 `end_turn`，仅在会话级
  `logs/kimi-code.log` 写入结构化失败原因。 Lingo 只读取当前回合新增日志中的明确
...
Lingo 对三种 Agent 复用同一套登录流程，但安装任务、安装状态、登录任务和登录状态
...
`kimi login` 生成并轮询， Lingo 不代管或复制 OAuth token。
