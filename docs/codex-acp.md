...
> 智灵原生代码会话（进程内 CodeWhale Engine，非 ACP 子进程）；多 Agent
> 结构、运行时来源和登录边界见 [`multi-agent-acp.md`](./multi-agent-acp.md)。

> 智灵原生会话的设计决策、开发节点与改动说明见
> [`code-native-agent.md`](./code-native-agent.md)。
> 本文说明当前 MVP 的使用、验证和发布方式。

pinvou3 在主页输入区提供“工作 / 代码”两种模式：“工作”保持原有智灵输入框，
“代码”可选 Codex / Claude Code / Kimi（ACP）或智灵原生。两类会话按最近更新时间
混排在左侧统一会话列表中，以各自 Agent 图标区分，不再占用单独的侧边栏入口。
ACP 会话仍使用独立的 ACP 事件、权限和持久化链路，不进入 CodeWhale `ChatView`；
智灵原生代码会话复用 CodeWhale Engine 与 `chat` 命令、`chat:*` 事件链路。原生
...
2. 启动 `./pinvou3-app/run-dev.sh`。智灵 会优先检测系统 Codex：存在且版本不低于
...
3. 在主页选择“代码”，输入框下方默认选择“临时会话”；直接发送首条消息时才创建
   ...
   智灵原生会话同样支持临时会话与项目目录两种工作区：绑项目后 LLM 直接在项目目录
...
`session/set_mode`。智灵 不再在每次 Prompt 前重复设置模式。用户确认的模式写入
`session-agents.json`，ACP `session/new`、`session/load` 或进程重连后会先重新应用，
再把会话标记为可发送。配置应用期间和 Turn 运行期间不能发送另一项配置修改。

ACP 的配置作用域是单个 session，不提供跨 session 默认值。智灵 会把用户成功选择的
...
应用被直接关闭时，旧进程中的 ACP Prompt 无法在新进程重新挂接。智灵 启动恢复会把
...
- `~/.pinvou3/session-agents.json`：pinvou 会话与 ACP session ID / model /
  用户确认配置的轻量索引；智灵原生代码会话在其中以 `code_session: true` 标记，
...
项目会话只在 `session-agents.json` 中保存 canonical absolute path，智灵 的 timeline、
...
原生 skills、MCP 与 Codex 自身会话记忆仍由 Codex 管理。智灵 不把自身记忆注入 Codex。
...
智灵 的「代码」模式。设置页「生效中配置」区展示 CLI 配置文件当前实际生效的
...
- Codex 的 config.toml 无明文 key 字段：key 由 智灵 在 spawn Codex 子进程时注入
...
- 智灵负责进程托管、ACP 事件还原、权限交互、时间线持久化和 UI。
- MVP 不向 Codex 注入智灵 bundle skill、MCP、知识库或 persona。
