...
**Lingo 压缩的一等目标是「最小化压缩次数」，不是「省 token」。**
...
云端大窗口用户不在乎这个； Lingo 在乎。**据此，评判任何压缩改动的唯一标准是：
...
|---|---|---|
| k | conservative 放大系数 | **1.5（实测精确**，三档 k_eff=1.500， Lingo 关闭 thinking 无偏移） |
...
按部署显式配置，最终取“档案值、 Lingo 进程级请求上限、窗口可容纳值”的较小值。底座只在
...
> 监控卡「上下文长度」行下显示琥珀色告警（中英文）；云端 /remote 不触发。纯前端零 fork。
...
| 会话 RAG（ Lingo 有 L1 基础） | 对话连续性/指代不能靠检索 | 同上 |
| 结构化状态（todo/notes） | 底座已部分覆盖，业务非长循环 | 同上 |
| vLLM `/tokenize` 精确计数 | 每 turn 多一次往返 + Lingo 专用 | 仅作回灌异常时的对账探针 |
...
**结论**：
1. **k=1.5 精确**：三档（262K/131K/65K）k_eff 全 =1.500。 Lingo 关闭 thinking，两把尺
