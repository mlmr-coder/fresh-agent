...
- **内容**：
  ...
  - `install_mcp_secret_resolver` 宿主钩子：mcp.json `${...}` 占位符、`env_headers`、`bearer_token_env_var` 解析先查宿主注册的进程内回调，未命中回落进程 env；嵌入宿主（ Lingo ）据此把 MCP secret 承载改为 keyring + 进程内注册表，消除运行时进程 env 写。
