...
impl KnowledgeService {
    pub fn boot(data_dir: PathBuf, model_dir: Option<PathBuf>) -> Result<ServiceBoot, String> {
        ...
        {
            store
                .set_meta("server_name", "Lingo Knowledge")
                .map_err(|error| error.to_string())?;
        }
        if store
        ...
        // v2 is claimed by the host Lingo application through an Owner device credential.
    ...
    pub async fn run_trash_retention_loop(self: Arc<Self>) {
        ...
        loop {
            ...
            match tokio::task::spawn_blocking(move || service.purge_expired_trash()).await {
                Ok(Ok(_)) => {}
                Ok(Err(error)) => eprintln!("Lingo Knowledge trash cleanup failed: {error}"),
                Err(error) => eprintln!("Lingo Knowledge trash cleanup task failed: {error}"),
            }
        }
    }
    ...
    pub fn server_info(&self) -> Result<ServerInfo, String> {
        Ok(ServerInfo {
            ...
            name: self
                ...
                .meta("server_name")
                .map_err(|error| error.to_string())?
                .unwrap_or_else(|| "Lingo Knowledge".to_string()),
...
mod tests {
    ...
    fn host_owner_claim_is_persisted_before_the_owner_token_hash_is_committed() {
        ...
        let error = service
            .provision_host_owner("Host Lingo", |_, _| Err("disk full".to_string()))
            .unwrap_err();
    ...
    fn host_owner_recovery_rotates_only_after_the_new_claim_is_persisted() {
        ...
        let error = service
            .recover_host_owner("Host Lingo", |_, _| Err("disk full".to_string()))
            .unwrap_err();
        assert_eq!(error, "disk full");
        assert_eq!(
