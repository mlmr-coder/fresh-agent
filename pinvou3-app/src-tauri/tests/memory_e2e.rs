...
fn memory_runtime_injects_effective_five_layer_memory_only() {
    ...
    let focus = TimedMemoryItem {
        ...
        text: "正在完善 Lingo记忆系统的自动写入机制。".to_string(),
        source: "test".to_string(),
        confidence: 0.91,
        created_at: now.to_rfc3339(),
        updated_at: now.to_rfc3339(),
...
async fn memory_llm_realistic_effect_snapshot() {
    ...
    let current_focus = memory::review_turn_candidates_with_llm(
        &bridge,
        &memory::TurnMemoryCapture {
            user: "我最近主要在推进 Lingo记忆系统，要把自动发现、确认和近期工作状态做稳定。"
                .to_string(),
            assistant: "明白，这属于你近期正在推进的 Lingo记忆系统工作。".to_string(),
            ..memory::TurnMemoryCapture::default()
        },
        session_id,
    )
    ...
    let delivery = memory::review_turn_candidates_with_llm(
        &bridge,
        &memory::TurnMemoryCapture {
            user: "帮我把 Lingo记忆系统设计文档改成可开发方案。".to_string(),
    ...
    if focus.is_empty() {
        ...
        assert!(
            focus
                .iter()
                .any(|item| item.text.contains("Lingo") || item.text.contains("记忆系统")),
...
async fn memory_llm_background_project_midterm_snapshot() {
    ...
    let project_background = memory::review_turn_candidates_with_llm(
        &bridge,
        &memory::TurnMemoryCapture {
            user: "也请记住一个长期项目背景：我长期维护Lingo，它是本地 AI 通用办公助手，重点关注桌面体验、记忆系统、本地模型和办公工作流。这里说的是我长期做的项目，不是当前运行环境。"
                .to_string(),
            assistant: "明白，这会作为你的长期项目背景候选，不会当作 Lingo当前运行环境。"
                .to_string(),
            ..memory::TurnMemoryCapture::default()
        },
        session_id,
    ...
    let current_focus = memory::review_turn_candidates_with_llm(
        &bridge,
        &memory::TurnMemoryCapture {
            user: "这两周我正在做 Lingo的记忆系统和设置页联动，重点验证中期记忆、项目记忆、用户背景能不能正确生成。"
                .to_string(),
            assistant: "明白，这是你近期正在推进的 Lingo记忆系统验证工作。".to_string(),
            ..memory::TurnMemoryCapture::default()
        },
        session_id,
    )
    ...
    assert!(
        work_context
            .iter()
            .any(|item| item.text.contains("Lingo") && item.text.contains("本地 AI")),
