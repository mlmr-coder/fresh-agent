...
fn voice_postprocess_prompt(mode: &str) -> &'static str {
    if mode == "edit" {
        r#"你是 Lingo 的语音编辑器。你的唯一职责是根据用户的语音修改指令，改写“当前输入框已有文本”。
...
- 明天发给团队。"#
        ...
        r#"你是 Lingo 的语音任务纠错器。你的唯一职责是把 ASR 文本纠正为用户原本想交给 Agent 执行的任务。
...
7. 英文实体、模型名、产品名和 API 名称要尽量标准化：GPT-5、Claude Sonnet、DeepSeek V3、REST API、PDF、Lingo。
...
最终文本："#
        ...
        r#"你是 Lingo 的语音听写整理器。你的唯一职责是把 ASR 文本纠正并整理为用户原本想输入到文本框里的内容。
