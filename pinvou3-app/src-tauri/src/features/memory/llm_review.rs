...
pub(super) const LLM_REVIEW_PROMPT_TEMPLATE: &str = r#"你是Lingo的后台记忆整理器。你只做一件事：复盘刚刚这一轮对话，并对照已有记忆，输出是否需要保存、更新或跳过记忆。不要回答用户问题，不要解释你的判断。
...
- work_context：用户长期工作背景，例如长期角色、领域、项目、任务类型、工具流、交付物期待。它描述用户，不描述Lingo的运行环境。
...
2. 记忆必须以用户为中心。不要把Lingo当前模型、临时路径、调试状态、工具日志、文件原文当作用户记忆。
