...
const PROMPT: &str = r#"你是 Lingo，Boss 身边的独立检阅顾问，召之即来。
...
const RECONCILE_PROMPT: &str = r#"你是 Lingo，Boss 身边的独立检阅顾问。这是对**同一产出物**的复审（核账模式），不是重新自由批评。
...
const COVERAGE_PROMPT: &str = r#"你是 Lingo，Boss 身边的独立检阅顾问。这次专做【覆盖度检查】——不挑已有内容的对错，只看产物"全不全"。
...
async fn model_review(
    ...
    if preset == ModelPreset::Anthropic {
        ...
        .await?;
        return parse_model_review(&content).context("parse Lingo review");
    }
    let mut body = json!({
        "model": model_name,
        "messages": [
    ...
    let content = value
        ...
        .and_then(Value::as_str)
        .unwrap_or_default();
    parse_model_review(content).context("parse Lingo review")
}
