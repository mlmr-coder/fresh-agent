...
/// 代码模式层（Lingo原生代码会话）：§工作环境（代码模式身份 + `{{PINVOU3_WORKSPACE_HINT}}`
...
Lingo界面语言为简体中文。跟随用户消息的语言回复;无法判断时用简体中文。\
代码、路径、工具名、URL 保持原样。";
    ...
    fn browser_mcp_entry_is_gated_to_work_mode_config() {
        ...
        if !crate::platform::capabilities::browser_product_enabled() {
            ...
            assert!(
                ...
                "the session-reserved browser name must always target the Lingo wrapper"
            );
            assert_eq!(
