...
fn preset_manifests() -> &'static [SkillManifest] {
    &[
        ...
        SkillManifest {
            ...
            description: "将结构化数据、表格汇总和业务指标转成符合 智灵 宿主体验的 HTML 可视化仪表盘。默认使用 Chart.js、无障碍 canvas、自定义图例、扁平配色，并通过 .html 产物卡交付。",
            icon: "LineChart",
        ...
        SkillManifest {
            ...
            description: "把散乱的技能（SKILL.md）、MCP 服务或它们的组合整理成 智灵 商店可导入的标准插件包：补 plugin.json、补 mcp/manifest.json、补 SKILL.md frontmatter、生成图标、校验命名与布局，最后产出目录或 zip。",
            icon: "Package",
        ...
        SkillManifest {
            ...
            description: "接入腾讯 ima OpenAPI，用本机凭据调用官方接口管理 IMA 笔记与知识库。凭据由 智灵 工具市场写入本机系统凭据，不需要在对话里粘贴 Token。",
            icon: "BookOpen",
