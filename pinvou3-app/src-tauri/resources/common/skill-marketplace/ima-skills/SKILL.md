---
name: ima-skills
description: Tencent IMA OpenAPI skill for notes and knowledge-base operations. Use after the user connects IMA in Lingo Plugin Center.
...
Lingo stores IMA credentials in the local system credential store. The native `ima_openapi` tool reads them only when it sends a request to the fixed official endpoint.
...
To verify access or perform any IMA operation, call `ima_openapi`. If it reports missing credentials, tell the user to connect "腾讯 ima" from the Lingo Plugin Center.
...
All calls go through Lingo's native `ima_openapi` tool. Pass only an allowlisted `api_path` and a JSON object in `body`:
