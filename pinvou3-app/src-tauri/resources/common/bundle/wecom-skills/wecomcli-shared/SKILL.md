...
description: 何时用:任何 wecomcli-* 技能准备执行 wecom-cli 命令前必须同时读取本技能(公共前置检查),本技能不处理具体业务请求。检查 CLI 已安装且版本 ≥1.1.0、企微凭证已授权(缺失时按Lingo代管口径引导,勿自行 npm 安装);获取机器人/授权真人身份(identity whoami);定义通用的 ID 类字段禁止外露输出约束。
metadata:
  requires:
    bins: ["wecom-cli"]
---
...
- 命令不存在、执行报错或版本号低于 `1.1.0` → **不要自行安装或升级**：`wecom-cli` 由Lingo应用代为安装与管理，随应用更新自动就位。此时提示用户在Lingo「工具商店 → 企业微信」卡片重新点「连接」，由Lingo触发安装/升级；完成后重新执行 `wecom-cli --version` 复查。
