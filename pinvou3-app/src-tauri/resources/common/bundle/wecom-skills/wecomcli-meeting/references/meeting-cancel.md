...
- **不预先按"是否本人创建"拦截取消**，由接口返回结果判断能否取消（取消前另须按 SKILL.md 规则 2 复述会议主题与时间并获确认）：返回空对象 `{}` 即成功；返回权限类错误则说明当前用户无权取消，告知用户并建议联系会议发起人
...
用户发起取消意图
    ...
    |   +-- repeat_rule 为空 → 非周期会议，进入取消前确认（复述会议主题与时间并获确认，见 SKILL.md 规则 2）
    |   +-- repeat_rule 非空 → 周期会议，终止操作，用文字告知用户："目前暂不支持取消周期会议，请在企业微信客户端对该会议进行取消操作"，禁止改为整系列直接 cancel 或其他变通方式
    |
    +-- 用户确认后执行 cancel（不论会议由谁创建，不提前拒绝）→ 依返回结果判断：
          +-- 返回空对象 {} → 取消成功，告知结果
...
| 异常情况 | 处理方式 |
|---------|---------|
| 接口返回无权取消（非发起人） | 复述确认后执行 cancel，依返回判断；返回权限错误时告知用户无权操作, 建议联系会议发起人 |
...
```
用户：帮我取消张三组织的评审会
→ 找到目标会议 → 不因非本人创建而提前拒绝；复述会议主题与时间并获用户确认后调用 cancel
