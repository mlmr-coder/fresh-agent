...
- 智灵运行环境始终可访问系统默认浏览器（`auth login` 走 `--no-browser` 输出授权 URL 后由用户在浏览器完成授权）。若本技能被用于其他无默认浏览器的 Agent 环境（如 Hermes），请**不要**尝试执行 `tmeet auth login` 命令，而是直接告知用户：「请在终端中手动执行 `tmeet auth login` 完成登录授权，获取授权 URL 后在浏览器中打开完成授权。」
- `TMEET_AGENT` 和 `TMEET_MODEL` 遥测环境变量由智灵宿主统一注入（值固定为 `FreshAssistant`），模型**不需要也不要自行设置或修改**。仅当本技能被用于智灵之外的 Agent 环境且宿主未注入时，才将当前使用的 agent 类型和模型名称分别写入这两个环境变量。
