...
- 交付物是可复制到邮件客户端的邮件成稿，不代表已发送，也不执行收件人查询、邮件发送、草稿箱或邮箱管理；实际邮件操作走 `lark-cli mail` 命令直连（lark-mail skill 未随包收录，可先 `lark-cli mail --help` 查看可用命令）。
...
查看、搜索、发送、回复或管理邮箱中的真实邮件属于 `lark-cli mail` 命令操作（lark-mail skill 未随包收录）；邮件系统说明、邮件数据分析、营销方案或把邮件作为信息来源时不触发本合同。若任务既要成稿又要实际发送，先按本合同形成并确认内容，再用 `lark-cli mail` 命令执行发送。
