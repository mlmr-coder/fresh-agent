...
OKR block 可用 XML 格式完整表达。创建前先用 `lark-cli okr +cycle-list` 确认可用周期（lark-okr skill 未随包收录，用法以 `--help` 为准）；创建时只写 root-only `<okr cycle-id="..."/>` 挂载已有 OKR，不构造 Objective/KR/Progress 子树。
