...
- **`document.new_blocks`**：本次操作新增的 block 列表（如画板）。`block_id` 可用于 `docs +update` 的 `--block-id` 做精确编辑；`block_token` 是资源块（如画板）的 token，画板后续操作用 `lark-cli whiteboard +export` / `+update`（lark-whiteboard skill 未随包收录，用法以 `--help` 为准）。
