...
展示格式与完整示例见 [`lark-calendar-room-find.md`](./lark-calendar-room-find.md) 的「输出格式」节；下方仅列本分支多选项叠加的差异：

- 多个候选时间块用 `[选项 N]` 递增分组，每个选项 = 一个 `+suggestion` 时间块 + 其对应的可用会议室编号列表，仍按「时间块 → 会议室候选」分行排版，严禁将时间与会议室放在同一行。
- 每个时间块后追加「（参会人均空闲）」标注，表示该时间块由 `+suggestion` 确认全员空闲。
- 结尾保留向用户征求「选项编号 + 会议室序号」的提示语。
