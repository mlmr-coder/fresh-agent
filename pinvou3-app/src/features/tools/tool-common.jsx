...
const AcFmtIcon = FileTypeIcon;
    ...
    const tsToolsData = [
      ...
      { id: 4, backendId: 'ima', imaOpenapi: true, title: '腾讯 ima', subtitle: '用 OpenAPI 操作 ima 笔记与知识库', category: 'docs', type: 'OpenAPI Skill', version: 'v1.1.8', latency: '云端', desc: '接入腾讯 ima OpenAPI Skill：通过 Lingo 内置的受控工具调用 ima.qq.com 官方 OpenAPI，支持笔记搜索/读取/创建/追加，以及知识库搜索、浏览、网页导入和内容添加。需要填写你自己的 Client ID 和 API Key，凭据只写入本机系统凭据，不进入对话、环境变量、仓库或 mcp.json。', icon: BookOpen, color: 'bg-gradient-to-b from-sky-500 to-indigo-600', installed: false, authRequired: true, configTitle: '连接腾讯 ima', configDescription: '凭据只保存在本机，用于启用 IMA OpenAPI Skill。', configDocUrl: 'https://ima.qq.com/agent-interface', configDocLabel: '获取 Client ID / API Key', configFields: [{ key: 'IMA_CLIENT_ID', label: 'Client ID', required: true, target: 'credential', secret: true, placeholder: 'Client ID' }, { key: 'IMA_API_KEY', label: 'API Key', required: true, target: 'credential', secret: true, placeholder: 'API Key' }], welcomeQueries: ['搜索我的 ima 知识库', '列出我有哪些 ima 笔记', '把这段内容新建为 ima 笔记', '在 ima 知识库里查产品方案'] },
    ...
    const tsToolWelcomeData = [
      { backendId: 'pptx', title: 'PPT 生成', category: 'docs', desc: '说“做个 PPT / 汇报”，AI 先列大纲让你确认，再按内容自动选主题（9 套）生成可编辑 .pptx——真·图表、自带封面缩略图，全程本地、数据不出机。首次安装会自动下载 python-pptx 依赖（需联网）。', icon: Presentation, color: 'bg-gradient-to-b from-orange-400 to-rose-500', welcomeQueries: ['做个 Q2 季度汇报 PPT', '帮我做一份产品介绍 PPT', '做个项目方案演示', '做个公司介绍 PPT'] },
    ];
