...
export function useComposerSuggestions({ text, setText, inputRef, sessionId, language, copy, disabled, onManageSkills, skillLabels, builtinSkillTitle }) {
  ...
  useEffect(() => {
    ...
    const request = kind === 'skills'
      ? invokeTauri('list_composer_skills', { language })
      : sessionId ? invokeTauri('list_composer_files', { sessionId }) : Promise.resolve([]);
    request.then(items => {
      if (!Array.isArray(items)) throw new Error('Invalid composer catalogue response');
      if (alive) setResult({ key: requestKey, items: (Array.isArray(items) ? items : []).map(item => kind === 'skills' ? {
    ...
    return () => { alive = false; };
  }, [kind, sessionId, language, requestKey, skillLabels, builtinSkillTitle]);

  const loaded = result?.key === requestKey;
  const entries = kind && loaded ? composerSuggestions(kind, result.items, token.query) : [];
  ...
  const menu = (
    <ComposerPopover open={open} onClose={close} triggerRef={inputRef} portal menuWidth={440}
      ...
      <div id={listId} role="listbox" aria-label={kind === 'files' ? copy.files : copy.skillsAndCommands}>
        ...
        {loaded && !entries.length && <div role="status" className="px-3 py-3 text-[13px] text-gray-400">{result.error ? copy.loadFailed : kind === 'files' ? copy.noFiles : copy.noMatches}</div>}
      ...
      <div className="mt-1 flex items-center justify-between gap-2 border-t border-black/5 px-3 pt-2 pb-1 text-[11px] text-gray-400 dark:border-white/10">
        ...
        {result?.error && <button type="button" onClick={() => setRevision(value => value + 1)} className="text-[#007AFF]">{copy.retry}</button>}
      </div>
    </ComposerPopover>
  );
  return { menu, onSelect, onKeyDown, close, inputProps: {
