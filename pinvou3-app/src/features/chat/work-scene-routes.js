...
const DOCUMENT_WRITING_CONTEXT = `Lingo 公文写作场景路由：
...
const DATA_VISUALIZATION_CONTEXT = `Lingo 数据可视化场景路由：
- 这是强制能力场景，不要按普通聊天、Excel 仪表盘或泛化可视化处理。
- 必须优先加载并使用数据分析可视化技能，技能 id/name 使用 visualizer / 数据分析可视化。
- 交付目标是可在 Lingo 产物预览中打开的 HTML 可视化仪表盘，默认使用 Chart.js。
...
const PPT_DESIGN_CONTEXT = `Lingo PPT 设计场景路由：
