...
export function composerSuggestions(kind, catalogue, query) {
  const entries = kind === 'files'
    ...
    : catalogue.map(skill => ({ id: `skill:${skill.name}`, title: `/${skill.title || skill.name}`, description: skill.description, aliases: [skill.name, ...(skill.aliases || [])], insertion: `/${skill.title || skill.name}`, kind: 'skill' }));
