...
const conversationZh = {
  completed:'已完成', failed:'失败', interrupted:'已中断', limitReached:'达到限制', processing:'处理中', processingActive:'正在处理', processed:'已处理', processDetails:'思考过程', working:'正在工作', worked:duration=>duration?`已工作 ${duration}`:'工作过程',
  toolBatch:count=>`${count} 项工具操作`, toolBatchRunning:count=>`${count} 项进行中`,
...
const codexZh = {
  ...
  activeHint:'工具调用、思考、计划和权限请求会按 ACP 原始语义展示，不进入 Lingo 原有工作会话消息框架。',
  nativeActiveHint:'工具调用、思考和选择确认按 Lingo Engine 语义展示，与工作会话共用同一引擎链路。',
  draftHint:'直接输入即可创建临时会话，也可以从输入框左下角选择项目目录。',
  nativeDraftHint:'直接输入即可创建临时会话，也可以从输入框左下角选择项目目录让 Lingo 直接在其中工作。',
  ...
  temporarySessionDesc:'使用 Lingo 管理的隔离目录', recentProjects:'最近项目',
  ...
  compactStart:'正在压缩上下文', compactDone:'上下文压缩完成', compactFail:'上下文压缩失败', compactAuto:'自动处理',
  nativeCompactInactive:'会话引擎尚未运行。请先发送一条消息，再压缩上下文',
  nativeUsageTitle:(used,pct)=>`上下文已用 ${used}${pct == null ? '' : `，占比 ${pct}%`}；接近上限时系统会自动压缩，点击可提前处理`,
  ...
  bridgeRepair:'请修复或重新安装 Lingo 。开发环境可运行 npm run prepare:codex-bridge。',
  ...
  finishAgentAuth:agent=>`请在浏览器中完成 ${agent} 授权；完成后 Lingo 会自动连接`,
  ...
  finishBrowserAuth:'请在浏览器中完成 ChatGPT 授权；完成后 Lingo 会自动连接',
...
const acpProvidersZh = {
  ...
  thirdPartyWarning:'Lingo 不会将您的 API Key 上传。',
...
export const dictZh = {
  ...browserZh,
        ...
        remoteKbPromoteOwner: '设为所有者', remoteKbDemoteOwner: '取消所有者', remoteKbPromoteOwnerConfirm: '将“{name}”设为所有者？该成员将可以管理共享与成员。', remoteKbDemoteOwnerConfirm: '取消“{name}”的所有者权限？', remoteKbModelTitle: '语义检索模型', remoteKbModelReady: '已可用', remoteKbModelMissing: '尚未下载', remoteKbModelDownloading: '正在下载，关闭 Lingo 后仍会继续', remoteKbDownloadModel: '下载', remoteKbModelDownloadingAction: '下载中', remoteKbPermanentDelete: '永久删除', remoteKbPermanentDeleteConfirm: '永久删除“{name}”？此操作无法撤销。',
        remoteKbOtherNetwork: '其他网络地址（可选）', remoteKbOtherNetworkPlaceholder: '例如 100.64.12.34:3210 或 cube.ts.net:3210',
        remoteKbCreateHost: '创建共享知识库', remoteKbHostCreated: '共享知识库已创建', remoteKbReconnectHost: '重新连接本机服务', remoteKbHostReconnected: '已恢复本机管理连接', remoteKbUpgradeHost: '升级服务', remoteKbHostUpgraded: '共享知识库服务已升级', remoteKbClientOutdatedTitle: '请先升级 Lingo', remoteKbClientOutdatedDesc: '当前 Lingo {appVersion} 低于本机服务 {serviceVersion}。为保护数据，不会降级服务。', remoteKbHostProgressCreateTitle: '正在创建共享知识库', remoteKbHostProgressReconnectTitle: '正在恢复本机连接', remoteKbHostProgressUpgradeTitle: '正在升级共享知识库', remoteKbHostProgressPrepare: '正在准备安装资源', remoteKbHostProgressInstall: '等待系统授权并安装服务', remoteKbHostProgressConnect: '正在连接本机服务', remoteKbHostProgressComplete: '已完成', remoteKbHostProgressFailed: '操作未完成', remoteKbHostProgressHint: '如系统弹出授权窗口，请完成授权。此过程通常需要几十秒。', remoteKbHostProgressRetry: '重试', remoteKbNoLanEndpoint: '未找到可供团队访问的局域网地址，请连接局域网后重试', remoteKbHostSettings: '主机设置', remoteKbHostSettingsDesc: '备份、恢复或管理本机托管服务。', remoteKbDangerZone: '停用与删除', remoteKbDangerZoneDesc: '停止托管会保留现有数据；删除服务和数据后无法恢复。', remoteKbBackupFileType: 'Lingo 备份', remoteKbBackup: '创建备份', remoteKbRestoreBackup: '恢复备份', remoteKbRecoveryTitle: '保存恢复码', remoteKbRecoveryDesc: '备份已加密。本机恢复无需恢复码；换一台主机时需要此恢复码。请与备份文件分开保存。', remoteKbCopyRecovery: '复制恢复码', remoteKbRestoreTitle: '恢复共享知识库', remoteKbRestoreDesc: '留空恢复码表示恢复到当前主机；输入恢复码表示迁移到这台主机。', remoteKbRecoveryPlaceholder: '换机迁移时粘贴恢复码（本机恢复请留空）', remoteKbSameHostMode: '本机恢复会保留当前服务身份和成员，仅恢复知识内容。', remoteKbMigrationMode: '换机迁移只导入知识内容，并创建新的服务身份；原成员需要重新加入。', remoteKbRestoreAction: '开始恢复', remoteKbRestoreConfirm: '恢复当前主机的知识内容？当前内容会被备份中的内容替换，服务身份和成员保持不变。', remoteKbMigrateConfirm: '将备份迁移到这台主机？当前内容会被替换，并创建新的服务身份和所有者。', remoteKbRestored: '共享知识库已恢复', remoteKbMigrated: '共享知识库内容已迁移到当前主机', remoteKbRemoveHost: '停止托管', remoteKbDeleteHost: '删除服务和数据', remoteKbRemoveHostConfirm: '停止本机的共享知识库服务？已有数据会保留，之后可以重新启用。', remoteKbDeleteHostConfirm: '永久删除本机的共享知识库服务和全部数据？此操作无法撤销。', remoteKbHostRemoved: '共享知识库服务已停止，数据已保留', remoteKbHostDeleted: '共享知识库服务和数据已删除',
        outputs: '产出物',
        ...
        kbOutSub: '你和 Lingo 一起做出来的东西，都在这里。',
        ...
        kbOutEmpty: '还没有产出物', kbOutEmptyHint: '和 Lingo 聊聊，一起做出来的东西会出现在这里。',
        ...
        kbModelPkgName: 'Lingo Embedding 模型', kbModelPkgSub: '多语言语义向量模型 · int8 量化 · 纯 CPU 本地运行',
        ...
        communityQrAlt: 'Lingo QQ 群二维码', communityQrPlaceholder: '二维码占位', communityChannelTag: '中文 QQ 社区',
        ...
        chatGreeting: '你好，今天想聊点什么？', artifacts: '产物与代码', placeholder: '今天帮你做些什么？', disclaimer: 'Lingo 可能会生成不准确的信息，请仔细核实。', ctxUsage: '上下文',
        ctxUsageTooltip: (percent, used, total) => `${percent}% · ${used} / ${total} 上下文已使用；接近上限时自动压缩`,
        ...
        uiComposerSuggestions: {
          hint: '@ 引用对话文件，/ 选择技能', files: '对话文件', skillsAndCommands: '选择技能', codeSkills: '代码可用技能', codeScopeHint: '仅显示代码模式已启用的技能',
          loading: '正在加载…', loadFailed: '加载失败，请重试', noFiles: '没有匹配的对话文件，可先用 + 添加文件', noMatches: '没有匹配的已启用技能', noCodeSkills: '没有匹配的代码可用技能，请到能力中心点击“管理代码技能”开启',
          keyboardHint: '↑ ↓ 选择 · Enter 插入 · Esc 关闭', manageSkills: '管理技能', manageCodeSkills: '管理代码技能', retry: '重试',
          skillCode: code => `技能编码 /${code}`,
          file: '文件', skill: '技能', codeAvailable: '代码可用', command: '指令',
        },
        ...
        // Lingo 检阅卡(PinvouSummonCard/Rows/Loading)。品/悟 字形是品牌、保留;周边文案随语言。
        pvLoadingPin: 'Lingo 品·查错检阅', pvLoadingWu: 'Lingo 悟·发散补全',
...
dictZh.uiSettingsView = {
  ...
  memorySummary: total => `Lingo 会记住你的偏好、工作背景和近期事项，让后续对话更容易接上上下文。已记录 ${total} 条有效记忆。`,
  memoryOffDesc: '开启后， Lingo 可以记住你的称呼、稳定偏好、工作背景和近期事项，减少重复说明。',
  memoryTurnOff: '关闭记忆',
  memoryTurnOn: '开启记忆',
  ...
  memoryOffNotice: '开启记忆后， Lingo 会在对话中使用这些信息，并自动整理新的长期记忆与近期记忆。',
  ...
  memoryRecentHint: '近期记忆会记录最近正在推进和刚完成的事情，用来帮助 Lingo 接上上下文。它会随时间自动淡出，你也可以手动归档或删除。',
...
desktopHint:'请先在桌面端安装 Obsidian 并创建笔记库，然后在这里重新检测。',
  ...
  codeAvailableOnly:'代码可用', codeAvailableOnlyHint:'只显示代码会话当前真正可以调用的技能', codeAvailable:'代码可用', codeUnavailable:'代码未启用', manageCodeSkills:'管理代码技能', manageCodeSkillsHint:'设置哪些技能可以在代码会话中使用',
  codeAvailabilityLoading:'读取代码状态…', codeAvailabilityUnknown:'代码状态未知', codeAvailabilityLoadFailed:'读取代码技能状态失败，请重试',
  emptyNoCodeAvailable:'没有代码可用技能', emptyNoCodeAvailableHint:'关闭筛选查看全部技能，再点“管理代码技能”开启需要的技能。',
...
visualizer: { title:'数据分析可视化', subtitle:'Chart.js 仪表盘 / 图表分析 / HTML 可视化', desc:'将结构化数据、表格汇总和业务指标转成符合 Lingo 宿主体验的 HTML 可视化仪表盘。默认使用 Chart.js、无障碍 canvas、自定义图例、扁平配色，并通过 .html 产物卡交付。', latency:'本地' },
'package-author': { title:'插件包标准化', subtitle:'把技能/MCP/函数整理成可上传的标准插件包', desc:'把散乱的技能（SKILL.md）、MCP 服务或它们的组合整理成 Lingo 商店可导入的标准插件包：补 plugin.json、补 mcp/manifest.json、补 SKILL.md frontmatter、生成图标、校验命名与布局，最后产出目录或 zip。', latency:'本地' },
...
Object.assign(dictZh.uiToolStore, {
  ...
  recycleBin:'回收站', recycleBinTitle:'能力回收站', recycleBinEmpty:'回收站是空的', recycleBinEmptyHint:'上传的连接器或技能包删除后会移到这里，可恢复或彻底删除。',
  ...
  connectorMore:name=>`${name}：更多操作`, connectorDetails:'查看详情', connectorManage:'管理', connectorDisconnect:'断开', connectorNothingToDelete:'尚未添加到本机，无需删除。', connectorAdded:'已添加', connectorNeedsAuth:'待授权', connectorNeedsConnection:'待连接', connectorAvailable:'未添加',
  removeConnector:'删除', removeConnectorTitle:name=>`从应用中删除「${name}」？`, removeConnectorHint:'删除本机连接配置、登录信息和配套能力，之后需要重新添加或连接。第三方账号和云端文件不会删除；上传的插件包会移入回收站。', connectorRemoved:name=>`已从应用中删除「${name}」`, connectorRemovedCleanupWarning:'本机内容已删除；第三方登录注销未完成，不影响从应用移除。', connectorRemoveFailed:'删除未完成，连接状态已刷新，请重试。',
  skillMore:name=>`${name}：更多操作`, removeSkill:'删除', skillNothingToDelete:'尚未安装到本机，无需删除。', builtinSkillCannotDelete:'内置技能不能删除，可在管理代码技能中关闭。', removeSkillTitle:name=>`从应用中删除技能「${name}」？`, removeSkillHint:'删除后，该技能会从工作、设计和代码会话的下一轮消息起停止生效；历史消息保留。上传的技能包会移入回收站，可在那里彻底删除。', removeCompanionSkillHint:'这个技能由连接器包提供。删除会同时移除对应连接器及其配套能力，并从所有会话的下一轮消息起停止生效；历史消息保留。', skillRemoved:name=>`已从应用中删除技能「${name}」`, skillRemovedWithConnector:'对应连接器及配套能力已一并删除。', skillRemoveFailed:'技能删除未完成，能力状态已刷新，请重试。',
