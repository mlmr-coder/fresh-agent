...
function Fixture() {
  ...
  const [reasoningText, setReasoningText] = useState('先阅读要求，再提取通知的基本结构。');
  ...
  window.__presentation = {
    ...
    fail: () => setFailed(true),
    setTokens,
    addTools: (count = 20) => setExtraTools(current => [...current, ...Array.from({ length: count }, (_, index) => ({
      id: `command-${current.length + index}`, type: 'command_execution', status: 'completed',
      tool: { name: 'exec_shell', title: 'private-command-marker', rawInput: { command: 'echo private-command-marker' }, rawOutput: { exit_code: 0 } },
    }))]),
    appendReasoning: () => { setReasoningStreaming(true); setReasoningText(value => value + '\n\n新的思考文字。'.repeat(30)); },
  ...
  const turn = {
    id: 'fixture', status: failed ? 'failed' : running ? 'running' : 'completed', startedAt: 1000, completedAt: running ? null : 17000,
    userText: '帮我整理一份事务型通知，说明应该包含哪些内容。',
    presentation: [
      { id: 'reason-1', type: 'reasoning', text: reasoningText, status: reasoningStreaming && running ? 'in_progress' : 'completed', startedAt: 1000, completedAt: 4000 },
      { id: 'tools', type: 'tool_group', items: [{ id: 'file', type: 'tool', status: failed ? 'failed' : running ? 'in_progress' : 'completed', startedAt: 4000, completedAt: 7000, tool: { name: 'File', title: '读取通知模板', kind: 'File', rawInput: { path: '通知.md' }, rawOutput: failed ? 'File not found' : '已读取模板内容' } }, ...extraTools] },
      { id: 'progress', type: 'agent_message', phase: 'commentary', status: 'completed', text: '模板已经读完，接下来整理通知正文。' },
