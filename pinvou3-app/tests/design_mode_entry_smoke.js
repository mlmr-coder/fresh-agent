...
function injectSource() {
  return `(function(){
    ...
    var HTML_CONTENT = '<!doctype html><html><body><main id="app"><section class="hero"><h1 class="hero-title">智灵 Design</h1><button class="primary">Start</button><a id="external-link" href="https://example.com/docs">Docs</a></section></main></body></html>';
...
(async () => {
  ...
  await page.keyboard.type('智灵 可视化编辑');
  await page.keyboard.press('Enter');
  await sleep(350);
  ...
  rec('设计面板可临时修改文案、字号、颜色和字体并记录 changes log',
    edited && edited.text === '智灵 可视化编辑' && edited.fontSize === '40px' &&
  ...
  if (frame) {
    await frame.waitForFunction(() => document.querySelector('h1.hero-title')?.textContent === '智灵 可视化编辑');
  }
  ...
  rec('切换产物再返回后恢复手工修改和 changes log',
    restoredAfterArtifactSwitch &&
      restoredAfterArtifactSwitch.text === '智灵 可视化编辑' &&
  ...
  rec('清空修改后恢复预览并清空 changes log',
    cleared && cleared.text === '智灵 Design' && cleared.fontSize === '32px' &&
