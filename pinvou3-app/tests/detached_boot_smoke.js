  ...
  return `(function(){
    ...
    window.__TAURI__ = { core: { invoke: async(cmd, args)=>{
      ...
      if (cmd === 'list_codex_acp_sessions') return [
        ...
        { id:'detached-native-42', title:'LingoCoding分离测试会话', agent_id:'pinvou', agent_name:'Lingo', workspace_kind:'temporary', workspace_path:'', workspace_available:true, updated_at:'2026-08-07T00:00:00Z' }
      ];
      if (cmd === 'load_session' && args && args.id === 'detached-native-42') return { metadata:{ id:'detached-native-42', title:'LingoCoding分离测试会话' }, messages:[{ role:'user', content:[{ type:'text', text:'DETACHED_NATIVE_HISTORY_OK' }] },{ role:'assistant', content:[{ type:'text', text:'DETACHED_NATIVE_REPLY_OK' }] }], artifacts:[] };
...
(async () => {
  ...
  try {
    ...
    console.error('FAIL: detached Lingo Coding session 未还原目标会话历史'); ok = false;
  }
  await page.evaluate(() => {
  ...
  try {
    ...
    console.error('FAIL: detached Lingo Coding session 未接收实时 chat 事件'); ok = false;
  }
  ...
  if (ok) { console.log('PASS: detached session/ACP及LingoCoding/monitor 启动与状态投影正常'); process.exit(0); }
  process.exit(1);
})();
