// 连接器技能包口径契约：把历次审查（2026-08-16 六轮）确认的Lingo适配规则固化为
...
for (const f of docs) {
  ...
  assert.ok(
    ...
    `${rel(f)}: 残留 npm 安装教学（-g/--global/@latest 均禁止，安装由Lingo代管）`,
  );
...
// 4) 上游宿主断言（Hermes/OpenClaw，含小写形态）必须以Lingo为锚。
...
for (const f of docs) {
  for (const line of read(f).split("\n")) {
    if (/(hermes|openclaw(?!_workspace))/i.test(line)) {
      ...
      assert.ok(
        !hostInProse || line.includes("Lingo"),
        `${rel(f)}: 上游宿主断言未锚定Lingo语境: ${line.trim()}`,
      );
    }
  }
}
