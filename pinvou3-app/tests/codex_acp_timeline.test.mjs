...
try {
  ...
  const events = [
    ...
    event(5, 'tool_call_update', { update: {
      toolCallId: 'tool-1', status: 'completed', rawOutput: { text: '# Lingo' },
    } }),
  ...
  assert.deepEqual(turn.tools[0].rawOutput, { text: '# Lingo' });
  ...
  'assistant avatars must use the Lingo and selected ACP Agent identity marks');
  ...
  // 原生（Lingo）车道底栏控件契约：仅 isNativeAgent 渲染、与工作/设计页共用同一套
  ...
  'the native lane must always expose factual delegated-agent cards and transcripts; product mode only controls the Lingo roster and reminder');
