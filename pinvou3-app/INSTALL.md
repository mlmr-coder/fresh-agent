...
Lingo 默认向本机 `http://127.0.0.1:8000/v1` 发送请求。接收方需**先自行启动 vLLM**，模型名建议包含 `_256k` 后缀（底座据此派生 256K 上下文窗口）。
...
- `--reasoning-parser qwen3` 建议按官方模型卡带上，由服务端把 thinking 内容解析到独立字段；缺省时 `<think>` 内容可能泄漏进正文。思考深度档位（reasoning effort）现由 Lingo 按模型配置透传（SavedModel.reasoning_effort），不再使用 `DEEPSEEK_REASONING_EFFORT` 环境变量。
