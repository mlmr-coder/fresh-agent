...
for cli in lark-cli wecom-cli dws; do
    if [ -x "$CONNECTOR_BIN/$cli" ]; then
        ...
        echo "  ⚠ $cli 尚未安装(首次在 Lingo 工具面板连接时会在线下载并校验)"
    fi
done
...
APP_BIN="$APP_SRC_TAURI/target/universal-apple-darwin/release/bundle/macos/Lingo.app/Contents/MacOS/pinvou3-tauri"
if [ -f "$APP_BIN" ]; then
